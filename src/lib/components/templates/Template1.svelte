<script>
	...
	let userHasContent = (!userPreferences.hide_testimonials && data?.expand?.testimonials?.length > 0 || !userPreferences.hide_projects && data?.expand?.projects?.length > 0|| !userPreferences.hide_clients && data?.expand?.clients?.length > 0) ;
</script>
